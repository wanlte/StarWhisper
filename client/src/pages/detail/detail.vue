<template>
  <view class="page-detail page-bg">
    <StarryBackground />
    <LoadingWrapper :loading="loading" text="星辰正在连通...">
      <template v-if="zodiac">
        ...
        <view class="hero content-layer">
          ...
          <text class="hero-name glow-text">{{ zodiac.name }}</text>
        ...
        <!-- Actions -->
        <view class="action-row content-layer">
...
.hero-emoji { font-size: 100rpx; display: block; margin-bottom: 16rpx; animation: floatY 4s ease-in-out infinite; }
.hero-name {
  font-size: 48rpx; font-weight: bold;
  background: linear-gradient(180deg, #F0C99A, #D4A574, #B8865A);
  -webkit-background-clip: text; -webkit-text-fill-color: transparent;
}
.hero-en { font-size: 26rpx; color: #888; display: block; margin-top: 8rpx; }
.hero-tags { display: flex; justify-content: center; gap: 16rpx; margin-top: 20rpx; flex-wrap: wrap; }
.tag {
  padding: 8rpx 20rpx; font-size: 22rpx; color: #D4A574;
  background: rgba(212, 165, 116, 0.08); border-radius: 20rpx;
...
.trait-desc { font-size: 26rpx; color: #B0B0C8; line-height: 1.7; }

.chip-wrap { display: flex; flex-wrap: wrap; gap: 12rpx; }
.chip { padding: 10rpx 24rpx; font-size: 24rpx; border-radius: 20rpx; }
.chip-green { background: rgba(76, 175, 80, 0.12); color: #81C784; }
.chip-pink { background: rgba(239, 83, 80, 0.10); color: #EF9A9A; }

.advice-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16rpx; }
.advice-item { padding: 18rpx; background: rgba(255,255,255,0.03); border-radius: 12rpx; border: 1rpx solid rgba(255,255,255,0.04); }
.advice-label { font-size: 22rpx; color: #888; display: block; margin-bottom: 6rpx; }
.advice-val { font-size: 26rpx; color: #DDD; }

.fun-fact { font-size: 26rpx; color: #B0B0C8; line-height: 1.7; }

.action-row {
  display: flex; gap: 24rpx; padding: 24rpx 0;
}
.action-row .btn { flex: 1; padding: 24rpx; font-size: 28rpx; border-radius: 24rpx; }
.disclaimer { display: block; text-align: center; font-size: 20rpx; color: #555; padding: 16rpx 0; }
